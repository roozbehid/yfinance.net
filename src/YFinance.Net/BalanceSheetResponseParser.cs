...
internal static class BalanceSheetResponseParser
{
    public static BalanceSheet Parse(ReadOnlyMemory<byte> json, string requestedSymbol, FinancialStatementFrequency frequency)
    {
        ...
        {
            return new BalanceSheet(requestedSymbol, frequency, [], []);
        }

        var prefix = frequency.ToWirePrefix();
        var symbol = requestedSymbol;
        var rawRows = new List<RawBalanceSheetLineItem>();
        ...
        if (rawRows.Count == 0)
        {
            return new BalanceSheet(symbol, frequency, [], []);
        }
        ...
        var lineItems = new BalanceSheetLineItem[rawRows.Count];
        for (var rowIndex = 0; rowIndex < rawRows.Count; rowIndex++)
        {
            ...
            lineItems[rowIndex] = new BalanceSheetLineItem(rawRow.Key, rawRow.CurrencyCode, values);
        }

        return new BalanceSheet(symbol, frequency, periods, lineItems);
    }

    private static ParsedBalanceSheetRow? ParseResultItem(JsonElement resultItem, string prefix)
    {
        string? symbol = null;
        string? type = null;
        string? propertyName = null;
        List<RawBalanceSheetValue>? values = null;
        ...
        return new ParsedBalanceSheetRow(symbol, new RawBalanceSheetLineItem(key, values));
    }
    ...
    private static List<RawBalanceSheetValue> ParseValues(JsonElement valuesArray)
    {
        var values = new List<RawBalanceSheetValue>();
        ...
        foreach (var item in valuesArray.EnumerateArray())
        {
            ...
            values.Add(new RawBalanceSheetValue(asOfDate, periodType, currencyCode, value));
        }
    ...
    private readonly record struct ParsedBalanceSheetRow(string? Symbol, RawBalanceSheetLineItem LineItem);

    private readonly record struct RawBalanceSheetLineItem(string Key, List<RawBalanceSheetValue> Values)
    {
        public string? CurrencyCode => Values.FirstOrDefault().CurrencyCode;
    }

    private readonly record struct RawBalanceSheetValue(DateOnly AsOfDate, string? PeriodType, string? CurrencyCode, decimal? Value);
}