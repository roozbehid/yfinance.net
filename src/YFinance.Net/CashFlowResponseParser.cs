...
internal static class CashFlowResponseParser
{
    public static CashFlow Parse(ReadOnlyMemory<byte> json, string requestedSymbol, FinancialStatementFrequency frequency)
    {
        ...
        {
            return new CashFlow(requestedSymbol, frequency, [], []);
        }

        var prefix = frequency.ToWirePrefix();
        var symbol = requestedSymbol;
        var rawRows = new List<RawCashFlowLineItem>();
        ...
        if (rawRows.Count == 0)
        {
            return new CashFlow(symbol, frequency, [], []);
        }
        ...
        var lineItems = new CashFlowLineItem[rawRows.Count];
        for (var rowIndex = 0; rowIndex < rawRows.Count; rowIndex++)
        {
            ...
            lineItems[rowIndex] = new CashFlowLineItem(rawRow.Key, rawRow.CurrencyCode, values);
        }

        return new CashFlow(symbol, frequency, periods, lineItems);
    }

    private static ParsedCashFlowRow? ParseResultItem(JsonElement resultItem, string prefix)
    {
        string? symbol = null;
        string? type = null;
        string? propertyName = null;
        List<RawCashFlowValue>? values = null;
        ...
        return new ParsedCashFlowRow(symbol, new RawCashFlowLineItem(key, values));
    }
    ...
    private static List<RawCashFlowValue> ParseValues(JsonElement valuesArray)
    {
        var values = new List<RawCashFlowValue>();
        ...
        foreach (var item in valuesArray.EnumerateArray())
        {
            ...
            values.Add(new RawCashFlowValue(asOfDate, periodType, currencyCode, value));
        }
    ...
    private readonly record struct ParsedCashFlowRow(string? Symbol, RawCashFlowLineItem LineItem);

    private readonly record struct RawCashFlowLineItem(string Key, List<RawCashFlowValue> Values)
    {
        public string? CurrencyCode => Values.FirstOrDefault().CurrencyCode;
    }

    private readonly record struct RawCashFlowValue(DateOnly AsOfDate, string? PeriodType, string? CurrencyCode, decimal? Value);
}